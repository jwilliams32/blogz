...
def require_login(): #not a request handler, will run for every request to check and see if user has logged in
    allowed_routes = ['login',  'signup', 'index', 'blog']#routes user don't have to login to see
...
def blog():
    if request.method == 'GET':
        ...
        if blog_id:
            ...
            return render_template('blogpost.html', title="Blog Post!", blog=entry)
        if user_id:

            user = User.query.get(user_id)
            return render_template('singleUser.html', title="Single User", user=user)#user.blogs)   
        else:
...
def newpost():
    ...
    if request.method == 'POST':#if incoming request is a post, todo submit
        ...
        new_post = Blog(title, body, user)#creates new task object
        ...
        if not title_error and not body_error:
            ...
            return redirect("/blog?blogid="+str(new_post.id))
        else:
