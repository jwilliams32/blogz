...
class User(db.Model):
    ...
    def __init__(self,username,owner_id):
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if username_boolean and password_boolean and verify_boolean and not existing_user:  
        ...
        return redirect('/newpost')
                ...
                #flash("User name or Password not 3-20 characters or Passwords do not match","error")
        return render_template('signup.html' password=password)#, username = username, password = password,
                            #verify = verify, username_error = username_error, 
...
def newpost():
    owner = User.query.filter_by(username=session['owner']).first()
