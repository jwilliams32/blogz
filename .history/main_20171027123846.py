...
def blog():
    if request.method == 'GET':
        ...
        if blog_id:
            #return redirect('/blogpost?id={0}'.format(blog.id))
            return render_template('blogpost.html', title="Blog Post!", blog=blog)#blog= Blog.query.get(blog_id))
        if user_id:

            user = User.query.filter_by(user_id)
            return render_template('singleUser.html', title="Single User", blog=blog)#user.blogs)   
        else:
