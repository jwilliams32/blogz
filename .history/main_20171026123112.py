...
def newpost():
    ...
    if request.method == 'POST':#if incoming request is a post, todo submit
        ...
        if not title_error and not body_error:
            ...
            blogs = Blogs.query.filter_by(blogs=blogs).all()
            return redirect("/blog?id="+str(id))
        else:
            return render_template('post.html', title_error=title_error,body_error=body_error, title=title, body=body,blog=blog)
    else:
